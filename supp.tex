...
Here we assume $\X$ has been standardized s.t. $\x_j\Tr\x_j = n$. Define $\Q_{\hat{S}_j}$ as $\I - \X_{\hat{S}_j}(\X_{\hat{S}_j}\Tr \X_{\hat{S}_j})^{-1} \X_{\hat{S}_j}\Tr$, the projection matrix onto the features selected by the lasso. For $\beta_j$ conditional on the selected features
\as{
  \begin{aligned}
...
The lasso can be formulated as a Bayesian regression model with a Laplace (double exponential) prior. In this case, the prior for $\beta_j$ is proportional to $\exp(-\frac{\tilde{n} \lambda} {\sigma^2} \abs{\beta_j})$. This prior is consistent with the value of $\lam$ using during estimation.
...
The piecewise defined distribution is made up of scaled left and right truncated Normal distributions. This can be leveraged and draws can be analytically obtained through a mapping onto the respective underlying unscaled Normal distributions, $\Norm(\mu \pm \lam, s^2)$. Note, the use of ``tails'' in what follows refers to the entirety of a distribution between 0 and $\pm \infty$. That is, the negative tail is any part of the distribution below 0 and the positive tail is any part greater than 0. With this defined, then more specifically the piecewise defined Laplace-Normal is made up of the negative tail of a right scaled Normal and the positive tail of a left scaled Normal. Accordingly, we will let the mass in the unscaled Normal tails be denoted $Pr_{-}$ and $Pr_{+}$ respectively and the mass in each of the tails of the Laplace-Normal be denoted $Post_{-}$ and $Post_{+}$ respectively. $Pr_{\pm}$ is trivial to compute with any statistical software. $Post_- = (C_{-} Pr_{-})/(C_{-} Pr_{-} + C_{+} Pr_{+})$ and similarly for $Post_+$. So, $Post_{\pm}$ is conceptually simple, although care must be taken to avoid numerical instability (see Algorithm~\ref{alg:quantile} for details).

With $Pr_\pm$ and $Post_\pm$ defined, we can compute a quantile by mapping the respective probability $p$ for the Laplace-Normal onto the probability $p^*$ for the corresponding Normal. Which Normal the quantile of interest ultimately comes from is determined based on $Post_{\pm}$. If $p \leq Post_{-}$, then $p$ would be mapped onto the negative tail of the right Normal. If $p > Post_{-}$, then $p$ would be mapped onto the positive tail of the left Normal.  For example, if $Post_{+} = 0.98$ and $p = 0.1$ then $p$ would be mapped onto the left Normal. The transformation to map a given probability from the Laplace-Normal depends on which tail the quantile resides in. This map is simply
$$
p^* = \frac{Pr_{\pm}}{Post_{\pm}} p.
$$

\noindent Once the respective probability is mapped, one can simply use the inverse of the Normal CDF that the probability was mapped to. That being said, there is a nuance worth pointing out. When mapping the probabilities, the step to determine which tail the respective quantile comes from occurs first. With this, the probability must be adjusted so that it refers to the probability between the quantile of interest and the respective tail (i.e., $1-p$ for the positive tail). After this, then the map can be applied and the quantile computed. Note, in practice the mapping and quantile computation are also performed on the log scale, but are described here on the non-log scale for conceptual clarity. Computing a quantile from a Laplace-Normal distribution is summarized in Algorithm~\ref{alg:quantile}.
...
\begin{algorithmic}[1]
  \Require
    $\tilde{\beta}_j,\;\sigma^2,\;\tilde{n};\lambda,\;p$ \Comment{mean, variance, effective sample size, penalty, probability of interest}
  \Statex
  \State // 1. Compute log negative/positive tail mass of right/left Normals
  ...
  \Statex
  \State // 2. Compute $Post_{-}, Post_{+}$
  \State $\ell_{-} \gets \log C_{-} + \log Pr_{-}$
  \State $\ell_{+} \gets \log C_{+} + \log Pr_{+}$
  \State $m \gets \max \lbrace \ell_{-}, \ell_{+} \rbrace$
  \State $c \gets m + \log\left(\exp(\ell_{-} - m) + \exp(\ell_{+} - m)\right)$
  \State $Post_{-} \gets \exp(\ell_{-} - c)$
  \State $Post_{+} \gets 1 - Post_{-}$
  \Statex
  \State // 3. Invert CDF on the appropriate unscaled Normal (on log scale)
  \If{$p \le Post_{-}$}
