...
\begin{figure}[hbtp]
  ...
  \caption{\label{fig:A1} Empirical coverage probabilities from a pairs bootstrap with equal tailed quantile intervals, computed using the simulation described in Section~\ref{Sec:coverage} with $n = 100$, $p = 101$, and coefficients $\boldsymbol{\beta}$ set to equally spaced quantiles of a Laplace distribution (shaded distribution). The curved line represents a smooth fit of the empirical coverage obtained using a binomial GAM. The dashed line is the coverage rate for all intervals constructed on the $1000$ simulated data sets. The solid black line indicates the 80\% nominal coverage rate. Compare to the right side of Figure~\ref{fig:1}.}
  \end{center}
\end{figure}
...
\begin{figure}[hbtp]
  ...
  \caption{\label{fig:C1} Empirical coverage probabilities for RL-P, computed using the simulation described in Section~\ref{Sec:coverage} where for each sample size, 1000 datasets were generated with coefficients $\boldsymbol{\beta}$ set to $p = 101$ equally spaced quantiles of a Laplace distribution (shaded distribution). The curved lines represent smoothed fits of the empirical coverages obtained using binomial GAMs. The dashed lines are the average coverage rates for all intervals constructed on the $1000$ simulated data sets. The solid black line indicates the 80\% nominal coverage rate.}
  \end{center}
\end{figure}
...
\begin{table}[hb]
  ...
  \caption{Additional information on the results for Selective Inference in the simulation described in Section~\ref{Sec:Comparison} where for each sample size, 1000 datasets were generated.}
  \label{tab:D1}
\end{table}
...
While the median interval width is 
...
\begin{table}[hbtp]
  ...
  \caption{\label{tab:E1} Coverage rates by magnitude of $\beta$ for RL-P using both the lasso and the MCP penalty applied to 1000 datasets generated under the Sparse 1 scenario (described in Section~\ref{Sec:distribution}) with $n = 100$ and $p = 101$. Prior to normalizing so that $SNR = 1$, Sparse 1 had $\bb_{1-10} = \pm(0.5, 0.5, 0.5, 1, 2)$ with the rest equal to zero. The nominal coverage rate is 80\%. }
\end{table}
...
\begin{figure}[htb!]
  \begin{center}
    ...
    \caption{\label{fig:F1} Results for pairs bootstrap (Ridge Bootstrap) and Bayesian posterior (Ridge Posterior) intervals for the simulation described in Section~\ref{Sec:boot-bias} where 1000 datasets were generated for each value of $p$ with $n = 200$ and $\bb$ set the $p$ equally spaced quantiles of a Normal distribution. A fixed values of $\lam$ was used so that the penalty/prior matched the generative distribution for $\bb$. Dashed lines give the average coverages across $p$ parameters and the solid curves are estimated coverages as functions of $\beta$.}
  \end{center}
\end{figure}
...
\begin{figure}[hbtp]
    ...
    \caption{\label{fig:F2} A decomposition of pairs bootstrap bias for the lasso on using a simulation using the simulation setup described in Supplement~\ref{sup:proof} where 1000 datasets were generate with $n = p = 100$, $\beta_A = 2$, $\beta_B = \beta_{N_1, \ldots, N_{98}} = 0$, $\cor(\x_A,\x_B) = 0.5$, and all $N$ variables are uncorrelated with $A$, $B$, and each other.}
    \end{center}
\end{figure}
...
\begin{table}[hbtp]
  ...
  \caption{\label{tab:G1} Results for simulation described in Supplement~\ref{sup:stability} where 1000 datasets were generated with $n = 50$, $p = 500$, $\beta_{1-4} = (0.25, 0.5, 1, 2)$ and the rest are zero. Original selection provides the empirical selection probabilities for the 4 parameters while Bootstrap stability gives the bootstrap estimate for selection.}
\end{table}
