...
if (interactive()) {
  saveRDS(bind_rows(res), glue("rds/{iterations}/laplace_relaxed_lasso_posterior.rds"))
} else {
  saveRDS(bind_rows(res), glue("code/rds/{iterations}/laplace_relaxed_lasso_posterior.rds"))
}
