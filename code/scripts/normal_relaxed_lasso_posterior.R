...
for (j in 1:length(ns)) {
  ...
  for (i in 1:iterations) {
    data <- gen_data_distribution(n = ns[j], p = 101, distribution = "normal", sigma = 10)
  ...
  res[[j]] <- bind_rows(intermediate_res) %>%
    mutate(n = ns[j], distribution = "normal")
}

if (interactive()) {
  saveRDS(bind_rows(res), glue("rds/{iterations}/normal_relaxed_lasso_posterior.rds"))
} else {
  saveRDS(bind_rows(res), glue("code/rds/{iterations}/normal_relaxed_lasso_posterior.rds"))
}
