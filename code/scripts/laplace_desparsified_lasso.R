...
for (j in 1:length(ns)) {
  ...
  for (i in 1:iterations) {
    ...
    suppressMessages({
      t <- system.time({
        intermediate_res[[i]] <- lp(data$X, data$y, alpha = 0.2, original = TRUE)
      })
    })
  ...
  res[[j]] <- bind_rows(intermediate_res) %>%
    mutate(n = ns[j], distribution = "laplace", method = "desparsified_lasso")
}

if (interactive()) {
  saveRDS(bind_rows(res), "rds/laplace_desparsified_lasso.rds")
} else {
  saveRDS(bind_rows(res), "code/rds/laplace_desparsified_lasso.rds")
}
