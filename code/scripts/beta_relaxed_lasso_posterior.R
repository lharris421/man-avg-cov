...
if (interactive()) {
  saveRDS(bind_rows(res), glue("rds/{iterations}/beta_relaxed_lasso_posterior.rds"))
} else {
  saveRDS(bind_rows(res), glue("code/rds/{iterations}/beta_relaxed_lasso_posterior.rds"))
}
