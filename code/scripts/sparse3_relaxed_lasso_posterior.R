...
for (j in 1:length(ns)) {
  ...
  for (i in 1:iterations) {
    data <- gen_data_distribution(n = ns[j], p = 101, distribution = "sparse 3", sigma = 10)
  ...
  res[[j]] <- bind_rows(intermediate_res) %>%
    mutate(n = ns[j], distribution = "sparse 3")
}

if (interactive()) {
  saveRDS(bind_rows(res), glue("rds/{iterations}/sparse3_relaxed_lasso_posterior.rds"))
} else {
  saveRDS(bind_rows(res), glue("code/rds/{iterations}/sparse3_relaxed_lasso_posterior.rds"))
}
