...
if (interactive()) {
  saveRDS(bind_rows(res), glue("rds/{iterations}/laplace_selective_inference.rds"))
} else {
  saveRDS(bind_rows(res), glue("code/rds/{iterations}/laplace_selective_inference.rds"))
}
