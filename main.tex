...
A normal likelihood and Laplace prior are not conjugate. However, the distribution of $\beta_j$ conditional on $\hat{S}_j $ can be shown to be a composition of right and left truncated normals where the truncation occurs at zero for right and left tails respectively. In this manuscript, we assume that $\X$ has been standardized s.t. $\x_j \Tr\x_j = n$ and that $\y$ has been centered. Then for $\beta_j$ (see Supplement~\ref{sup:A} for details),
\al{eq:fcp}{
...
This formulation is attractive because it allows efficient computation of quantiles. This consists of first determining which normal distribution (left or right tail) the probability corresponds to, then calculating the quantile from the corresponding normal distribution. Again, full details are provided in Supplement~\ref{sup:A}.
...
The shape of a Laplace-Normal distribution varies considerably based on the relative size of $\mu$ and $\lam$ which has direct implications on the resulting intervals. Intervals arising from Laplace-Normal distributions are often skewed away from zero unless $\mu$ is small or large relative to $\lam$, in which cases the intervals are more symmetric. Figure~\ref{fig:0} gives examples of the densities for 4 Laplace-Normal distributions with increasing values of $\mu$ = 0.1, 0.3, 0.6 and 1. For this figure, $\lam = 0.5$ and $s^2 = 1$. This figures highlights that the distribution is made up of two scaled truncated Normal distributions. The solid portion of each scaled Normal density is what is ``used'' in the whereas the dotted portion is the ``unused'' portion.

\begin{figure}[hbt!]
    \begin{center}
    \includegraphics[width=0.6\linewidth]{figure0}
    \caption{\label{fig:0} Examples of four Laplace-Normal distributions with increasing likelihood means ($\mu$ = 0.1, 0.3, 0.6, 1) and fixed values of $s^2 = 1$ and $\lambda = 0.5$.}
    \end{center}
\end{figure}

In all four panels, the density is continuous but non-differentiable at zero. When $\mu < \lam$, the distribution has a sharp spike at zero for the mode. The larger $\mu$ is relative to $\lam$, the more the distribution resembles that of a Normal distribution. In fact, if $\mu$ is large enough relative to $\lam$, the distribution is \emph{essentially} a Normal distribution. 
