...
\begin{figure}[htb!]
  \begin{center}
    ...
    \caption{\label{fig:1}
      \textbf{Both:} The solid black line indicates the 80\% nominal coverage rate. \textbf{Left:} Exact coverage probabilities across a range of $\theta$ values for credible intervals constructed with a $\Norm(\theta, 1)$ likelihood and a $\Norm(0, 1)$ prior (shaded distribution). The dashed line indicates the average coverage with respect to the prior distribution. \textbf{Right:} Empirical coverage probabilities from the lasso HDI method (RL-P) introduced in Section~\ref{Sec:methods}, computed using the simulation described in Section~\ref{Sec:coverage} with $n = 100$, $p = 101$, and coefficients $\boldsymbol{\beta}$ set to equally spaced quantiles of a Laplace distribution (shaded distribution). The curved line represents a smooth fit of the empirical coverage obtained using a binomial GAM. The dashed line is the coverage rate for all intervals constructed on the $1000$ simulated data sets.}
  \end{center}
\end{figure}
...
In Section~\ref{Sec:methods}, we propose a new method, and in Section~\ref{Sec:results} we demonstrate that the resulting intervals, although not satisfying classical coverage requirements, attain strong empirical performance with respect to the average coverage criterion. We end this section with a short theorem making the explicit connection between Bayesian credible intervals and average coverage.
...
where $\tilde{n} = \x_j \Tr \Q_{\hat{S}_j} \x_j$, $C_{1} = \exp(\tilde{\beta}_j \lambda \tilde{n}/\sigma^2)$ and $C_{2} = \exp(-\tilde{\beta}_j \lambda \tilde{n}/\sigma^2)$. $\tilde{n}$ can be interpreted as the effective sample size.
...
The Relaxed Lasso Posterior intervals are available through \texttt{intervals(fit, relaxed = TRUE)} in the current version of the R package \texttt{ncvreg} (3.16.0) where \texttt{fit} is an \texttt{ncvreg} or \texttt{cv.ncvreg} object.
...
\begin{figure}[hbt!]
    \begin{center}
    \includegraphics[width=0.6\linewidth]{figure0}
    \caption{\label{fig:0} Examples of four Laplace-Normal distributions with increasing likelihood means ($\mu$ = 0.1, 0.3, 0.6, 1) and fixed values for the likelihood variance $\hat{\sigma}^2 / \tilde{n} = 1$ and regularization parameter $\lambda = 0.5.$ The rate parameter for the Laplace prior is  $\tilde{n}\lam / \hat{\sigma}^2 = 0.5.$ The depiction shows that the Laplace-Normal can be represented piecewise by two scaled truncated Normal distributions.}
    \end{center}
\end{figure}
...
Low coverage for large values of $\beta$ arises from the bias introduced by the lasso penalty. However, as $n$ increases, the value of $\lambda_{\CV}$ decreases, reducing the bias. Consequently, coverage becomes flatter and closer to the nominal level across the range of $\beta$, as shown in Supplement~\ref{sup:alt_ns}.

Bias can also be reduced by choosing an alternative penalty. For example, the method described in Section~\ref{Sec:methods} can be extended to the Minimax Concave Penalty (MCP) to construct Relaxed MCP Posterior (RM-P) intervals. These intervals exhibit coverage that remains closer to the nominal level for larger values of $\beta$ compared to the RL-P intervals (Supplement~\ref{sup:MCP}). This demonstrates that the phenomenon of uneven coverage across $\beta$ is not inherent to the proposed interval approach, but rather, reflects properties of the penalty used in estimation.

\subsection{Robustness for Average Coverage}
\label{Sec:robustness}

We will now shift our attention to the robustness of the RL-P method under alternative scenarios. We begin by assessing coverage when there is correlation among the predictors. Next, we examine the behavior of RL-P under various distributions of $\beta$. Finally, we look at how the coverage changes across the range of $\lambda$ values.

\subsubsection{Correlation}
\label{Sec:correlation}

Figure~\ref{fig:2} illustrates the coverage of the RL-P intervals as the level of correlation, $\cor(\x_i, \x_j) = \rho^{\abs{i-j}}$, increases. Otherwise, the simulation design is the same as in Section~\ref{Sec:results}; in fact, the design is exactly the same for $\rho = 0$. The violin plots provide the distributions of average coverages across 1000 simulated data sets for four values of $n$ and three values of $\rho$. For each $n$, the amount of correlation is increased from $\rho = 0$ to $0.5$ to $0.8$.
...
\begin{figure}[htb!]
  \begin{center}
    ...
    \caption{\label{fig:2} Results for the simulation described in Section~\ref{Sec:correlation}. The violin plots display the distribution of average coverages across 1000 simulated datasets for the RL-P method and across three different levels of autoregressive correlation among the covariates, $\rho = 0 \text{ (no correlation)}, 0.5, 0.8$. For this simulation, $\bb$ is set to $p = 101$ equally spaced quantiles of a Laplace distribution. The horizontal black line provides reference for the 80\% nominal coverage rate.}
  \end{center}
\end{figure}
...
\begin{table}[htb!]
  ...
  \caption{\label{tab:1} Average coverages from the simulation described in Section~\ref{Sec:distribution} which examines the coverage behavior of RL-P intervals under various distributions for $\bb$. For each respective distribution, $\bb$ is set to $p = 101$ equally spaced quantiles. The nominal coverage rate is 80\%.}
\end{table}
...
\begin{figure}[htb!]
  \begin{center}
    ...
    \caption{\label{fig:3} Displays relative coverage (estimated coverage minus 80\% nominal coverage) for RL-P intervals across a range of $\lambda$s per the simulation described in Section~\ref{Sec:lambda}. A Binomial GAM was used to estimate coverage as a smooth function of the $|\beta|$ and $\lam$. A total of 1000 datasets were simulated with $n = 100$ and $\bb$ set to be $p = 101$ equally spaced quantiles of a Laplace distribution. The solid black lines indicate the center 75\% of $\lam_{\CV}$s selected on the 1000 simulated datasets. The dashed black line indicates the median $\lambda_{\CV}$ and the blue line represents the value of $\lambda$ which provided coverage closest to that of nominal.}
  \end{center}
\end{figure}
...
\begin{figure}[htb!]
  \begin{center}
    ...
    \caption{\label{fig:4}
      Results for Ridge Posterior intervals (left) and RL-P intervals (right) for the simulation described in Section~\ref{Sec:Ridge} where 1000 datasets were generated with $p = 100$, 1 signal variable $\beta_{A} = 1$, and $\cor(\x_{A}, \x_{B}) = .99$ but all of the $N$ (noise) variables uncorrelated with $A$, $B$, and each other. \textbf{Top:} All 1000 intervals are plotted for variables $A$, $B$, and $N1$ across the 1000 simulations, sorted by their midpoint, with those colored red that did not contain the true coefficient value (indicated by the horizontal dashed gold line). \textbf{Bottom:} Shows a single example of intervals for one (randomly selected) dataset for the first 20 variables. 
    }
  \end{center}
\end{figure}
...
\begin{figure}[htb!]
  \begin{center}
    ...
    \end{minipage}
    \caption{\label{fig:5} Results from the simulation described in Section~\ref{Sec:Comparison} where 1000 datasets were generated with $\bb$ set to be $p = 101$ equally spaced quantiles of a Laplace distribution (shaded distribution in left plot) for 3 different sample sizes, $n = 50, 100, 400$. \textbf{Left:} Results for $n = 100$ with coverage modeled as a smooth function of $\beta$ using Binomial GAMs. The dashed lines represent the average coverage for each method across all 1000 datasets and the solid black line indicates the nominal coverage rate. \textbf{Right:} The top provides violin plots of average coverages, the middle is a bar plot of the the median interval widths, and the bottom is a bar plot of the average run times, across all 1000 simulated datasets for each sample size. The y limits have been truncated for the median width from 150 to 20.}
  \end{center}
\end{figure}

We conducted a simulation study to compare these three methods; the setup is identical to that described in Section~\ref{Sec:coverage}. For each software package, their default options were used. Notably, this means that for de-sparsified lasso's implementation in the \texttt{hdi} package, $\lam$ is set using the 1SE rule from cross-validation, whereas for Selective Inference and RL-P, $\lam$ was set at the value which minimizes CV error. \logan{Additionally, for Selective Inference, the default \texttt{sigma = NULL} is used --- see Supplement~\ref{supp:sel-inf-add} for results estimating $\sigma$ with \texttt{estimateSigma}}
...
\begin{figure}[htb!]
  \begin{center}
    ...
    \caption{\label{fig:7} Intervals produced by three different methods for the 30 genes with the largest absolute point estimates in the gene expression dataset described in Section~\ref{Sec:Scheetz2006}.}
  \end{center}
\end{figure}
