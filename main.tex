...
where $\y$ is a length $n$ vector of independent outcomes, $\X$ is an $n \times p$ matrix of features, $\bb$ is a length $p$ vector of regression coefficients, and $\lambda$ is a regularization parameter controlling the amount of penalization. Note that the objective function involves the addition of the $L_1$ penalty, $\lambda\norm{\bb}_1 = \lambda \sum_{j = 1}^p |\beta_j|$, to the squared error loss. This typically results in sparse estimates for some of the regression coefficients (i.e., $\bh_j = 0$) depending on the choice of the regularization parameter $\lambda$. \logan{The ability to use the lasso for} both variable selection and estimation is particularly attractive, especially in scenarios where both predictive accuracy and interpretability are important. The lasso \logan{is particularly effective} in cases where the number of features is large and the underlying model is sparse \citep{HTF2009}, but \logan{it has been successfully applied} in a wide variety of settings.
...
In Section~\ref{Sec:methods}, we propose a new method, and in Section~\ref{Sec:results} we \logan{demonstrate that the resulting intervals, although not satisfying classical coverage requirements, result in strong empirical performance} with respect to the average coverage criterion. We end this section with a short theorem making the explicit connection between Bayesian credible intervals and average coverage.
...
\begin{figure}[hbt!]
    ...
    \caption{\label{fig:0} Examples of four Laplace-Normal distributions with increasing likelihood means ($\tilde{\beta}_j$ = 0.1, 0.3, 0.6, 1) and fixed values for the likelihood variance $\hat{\sigma}^2 / \tilde{n} = 1$ and regularization parameter $\lambda = 0.5.$ The rate parameter for the Laplace prior is  $\tilde{n}\lam / \hat{\sigma}^2 = 0.5.$ The depiction shows that the Laplace-Normal can be represented piecewise by two scaled truncated Normal distributions.}
    \end{center}
\end{figure}
...
We will now shift our attention to the robustness of the RL-P method under alternative scenarios. We begin by assessing coverage when there is correlation among the predictors. Next, we \logan{examine the coverage performance using} RL-P under various distributions of $\beta$. Finally, we look at how the coverage changes across the range of $\lambda$ values.
...
% \logan{Additionally, for Selective Inference, the default \texttt{sigma = NULL} is used --- see Supplement~\ref{supp:sel-inf-add} for results estimating $\sigma$ with \texttt{estimateSigma}}
...
\begin{figure}[htb!]
  \begin{center}
    ...
    \caption{\label{fig:7} Intervals produced by RL-P, Selective Inference, and the Desparsified Lasso for the 30 genes with the largest absolute point estimates in the gene expression dataset \citep{Scheetz2006} described in Section~\ref{Sec:Scheetz2006}.}
  \end{center}
\end{figure}
